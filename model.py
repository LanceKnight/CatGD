...
from models.GCN.GCNet import GCNet
...
class GNNModel(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if gnn_type == 'spherenet':
            ...
            self.gnn_model = GCNet(in_channels=args.in_channels,
                                   hidden_channels=args.hidden_channels,
                                   num_layers=args.num_layers,
                                   out_channels=args.out_channels)
        else:
        ...
        self.lin1 = Linear(out_dim+391, args.ffn_hidden_dim)
        self.lin2 = Linear(args.ffn_hidden_dim, args.task_dim)
        self.ffn = Linear(out_dim+391, args.task_dim)
    ...
    def forward(self, data):
        ...
        cat_embedding = torch.cat((graph_embedding, data.bcl_feat), 1)
        prediction = self.lin2(self.activate_func(self.dropout(self.lin1(cat_embedding))))
    ...
    def add_model_args(gnn_type, parent_parser):
        ...
        if gnn_type == 'spherenet':
            ...
            GCNet.add_model_specific_args(parent_parser)
        else:
