...
def prepare_data(args, enable_pretraining=False, gnn_type=None):
    """
...
def actual_training(model, data_module, use_clearml, gnn_type, args):
    ...
    prog_bar=TQDMProgressBar(refresh_rate=50)
...
if __name__ == '__main__':
    ...
    print(f'========================')
    print(f'Running model: {gnn_type}')
    print(f'========================')
    ...
    with open(filename, 'w') as out_file:
        use_clearml = False
        if use_clearml:
            task = Task.init(project_name=f"JCIM/schnet-benchmark",
